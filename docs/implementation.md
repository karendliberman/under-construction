...
**Spend controls, first.** Two distinct things, and it matters which is which. *Corrected September 2026:* the Anthropic console's spend limit is a **hard stop**, not the mere alert this paragraph used to claim — Settings > Billing, and requests past it return HTTP 400 `invalid_request_error`, with each tier also carrying an automatic monthly cap that returns 429 `enforced_spend_limit_reached`. The **app-side cap** still earns its place, computing running cost from `generation_nodes` and refusing new jobs above a threshold: the console's limit is monthly, org-wide, and fails jobs mid-run, whereas ours is a rolling window plus a per-job breaker that refuses before claiming. Set the console limit above the app-side one so the graceful refusal comes first. Set both *before the first API call*. This matters more with an agent than it did with a single call: an agent that decides to re-read the facts four times costs several times as much, and the variance is the point of using one. A retry loop overnight is a genuinely expensive and entirely avoidable mistake.
